...
.PHONY: help install test run encrypt-data decrypt-data

help: ## Show available targets and descriptions
	@awk 'BEGIN {FS = ":.*## "; printf "Usage:\n  make <target>\n\nTargets:\n"} /^[a-zA-Z_-]+:.*## / {printf "  %-10s %s\n", $$1, $$2}' $(MAKEFILE_LIST)

install: ## Install required R packages (shiny, shiny.router, testthat, openssl)
	@$(RSCRIPT) -e "install.packages(c('shiny', 'shiny.router', 'testthat', 'openssl'), repos = 'https://cloud.r-project.org')"
